...
def hash_passphrase(passphrase: bytes, hash_func: Any):
...
def localize_key(key: bytes, engineid: bytes, hash_func: Any):
    return hash_func(key + engineid + key).digest()


# RFC3414: A.2.1
def hash_passphrase_md5(passphrase: str):
    return hash_passphrase(passphrase.encode(), md5)


# RFC3414: A.2.2
def hash_passphrase_sha(passphrase: str):
    return hash_passphrase(passphrase.encode(), sha1)


def hash_passphrase_sha224(passphrase: str):
    return hash_passphrase(passphrase.encode(), sha224)


def hash_passphrase_sha256(passphrase: str):
    return hash_passphrase(passphrase.encode(), sha256)


def hash_passphrase_sha384(passphrase: str):
    return hash_passphrase(passphrase.encode(), sha384)


def hash_passphrase_sha512(passphrase: str):
    return hash_passphrase(passphrase.encode(), sha512)


def localize_key_md5(key: bytes, engineid: bytes):
    return localize_key(key, engineid, md5)


def localize_key_sha(key: bytes, engineid: bytes):
    return localize_key(key, engineid, sha1)


def localize_key_sha224(key: bytes, engineid: bytes):
    return localize_key(key, engineid, sha224)


def localize_key_sha256(key: bytes, engineid: bytes):
    return localize_key(key, engineid, sha256)


def localize_key_sha384(key: bytes, engineid: bytes):
    return localize_key(key, engineid, sha384)


def localize_key_sha512(key: bytes, engineid: bytes):
    return localize_key(key, engineid, sha512)


# RFC3414: 6.3.1
def authenticate_md5(auth_key: bytes, msg: bytes):
...
# RFC3414: 7.3.1
def authenticate_sha(auth_key: bytes, msg: bytes):
    mac = hmac.new(auth_key, msg, sha1)
    d = mac.digest()
    return d[:12]


def authenticate_sha224(auth_key: bytes, msg: bytes):
    mac = hmac.new(auth_key, msg, sha224)
    d = mac.digest()
    return d[:16]


def authenticate_sha256(auth_key: bytes, msg: bytes):
    mac = hmac.new(auth_key, msg, sha256)
    d = mac.digest()
    return d[:24]


def authenticate_sha384(auth_key: bytes, msg: bytes):
    mac = hmac.new(auth_key, msg, sha384)
    d = mac.digest()
    return d[:32]


def authenticate_sha512(auth_key: bytes, msg: bytes):
...
class Auth:
    hash_passphrase: Callable[[str], bytes]
    localize: Callable[[bytes, bytes], bytes]
    auth: Callable[[bytes, bytes], bytes]
    sz: int
